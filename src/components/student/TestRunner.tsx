...
type SetInfo = { id: string; question_count: number; time_limit_sec: number | null; pass_mark: number; is_active: boolean }

/**
 * Runs one test end to end: intro -> question by question -> submit -> result.
 * Shared by both entry points into a test (a topic test at
 * learn/[slug]/[lessonId]/take/[kind], and a subject-wide quiz at
 * learn/[slug]/subject-quiz/[id]) so there is exactly one implementation of
 * the secure start/submit flow, not two copies that could drift apart.
 */
export default function TestRunner({
  title, backHref, backLabel, onLoadSet,
}: {
  title: string
  backHref: string
  backLabel: string
  /** Loads the question_sets row this runner needs, INCLUDING its real id.
   *  Kept as a prop (rather than an id prop) so each entry point can apply
   *  its own extra rule (e.g. "all topics complete") before handing control
   *  to the runner, and so the runner always uses the id it actually loaded
   *  rather than one the caller separately constructed (which could drift
   *  out of sync with what onLoadSet found). */
  onLoadSet: () => Promise<SetInfo | null>
}) {
  ...
  if (phase === 'not_ready') return (
    <div className="max-w-2xl mx-auto text-center py-16 bg-white border border-[#E8E0D0] rounded-2xl my-10 shadow-md px-6">
      ...
      <h1 className="text-[#2D2416] text-xl font-black mb-2">{title} isn't ready yet</h1>
      <p className="text-sm text-[#8B7355] mb-6">Check back once questions have been added.</p>
      <Link href={backHref} className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm text-white bg-[#4F7C82] hover:bg-[#3d6068] transition-all">
  ...
  if (phase === 'intro') return (
    <div className="max-w-xl mx-auto py-10 px-6">
      <div className="text-center mb-8">
        <h1 className="text-slate-900 text-2xl font-black tracking-tight mb-1">{title}</h1>
      </div>
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-10 text-center">
      <p className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-6">{title} Result</p>
      ...
      <div className="flex gap-3 justify-center flex-wrap">
        ...
        <Link href={backHref} className="px-6 py-3 rounded-full font-black text-xs uppercase text-white bg-[#C89B5A] shadow-md">
          {backLabel}
        </Link>
      </div>
    </div>
